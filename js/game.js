...
const audioNames = [(audioEat = new Audio()), (audioTurn = new Audio()), (audioDead = new Audio()), (audioHit = new Audio())];
...
function checkBorder() {
  // A function that checks for going beyond the border
  // x
  if (snake.x < 0) {
...
function withoutBorder() {
  // A function that checks if the snake is touched border
  if (snake.x < 0) {
    audioPlay("dead");
    restart();
  } else if (snake.x >= canvas.width) {
...
function drawSnake() {
  ...
  snake.body.forEach((e, index) => {
    ...
    if (diff === "Hard") {
      // if snake has touched the bomd, reduce the length of the snake
...
function snakeStyles(e, index) {
  if (index === 0) {
    // for the first snake element(head)
...
document.addEventListener("keydown", (e) => {
  if (e.keyCode == 87 || e.keyCode == 38) {
    ...
    // A (left) or arrow left
    // turnLeft();
  } else if (e.keyCode == 83 || e.keyCode == 40) {
    // S (down) or arrow down
    // turnDown();
  } else if (e.keyCode == 68 || e.keyCode == 39) {
    // D (right) or arrow right
    // turnRight();
  }
});
